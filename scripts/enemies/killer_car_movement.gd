...
func _update_wheels(delta: float, forward_speed: float) -> void:
	front_left_steer.rotation.y = steering_angle
	front_right_steer.rotation.y = steering_angle

	var wheel_roll := forward_speed * delta / wheel_radius
	front_left_wheel.rotate_object_local(Vector3.UP, -wheel_roll)
	front_right_wheel.rotate_object_local(Vector3.UP, -wheel_roll)
	rear_left_wheel.rotate_object_local(Vector3.UP, -wheel_roll)
	rear_right_wheel.rotate_object_local(Vector3.UP, -wheel_roll)